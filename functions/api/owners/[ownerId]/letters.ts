...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  // 追加: 無い owner は 404 に
  const exists = await env.DB.prepare('SELECT 1 FROM owners WHERE id=?').bind(ownerId).first();
  if (!exists) return json({ error: 'not found' }, 404);

  const u = new URL(request.url);
  const showInbox = u.searchParams.get('inbox') === '1' || u.searchParams.get('includeInbox') === '1';

  let sql = 'SELECT id, content, status, created_at, updated_at FROM letters WHERE owner_id=?';
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  if (!content || content.length > 200) return json({ error: 'invalid content' }, 400);

  await env.DB
    .prepare('INSERT INTO letters (owner_id, content) VALUES (?, ?)')
