...
app.post(
  ...
  async (c) => {
    ...
    await c.env.DB.prepare(
      ...
      .bind(ownerId, displayName ?? ownerId, salt, hash)
      .run();
...
app.post(
  ...
  async (c) => {
    ...
    const row = await c.env.DB.prepare(
      ...
      .bind(ownerId)
      .first<{ pw_salt: string; pw_hash: string }>();
...
app.post("/api/owners/:ownerId/letters/:id/status", requireAuth, async (c) => {
  ...
  await c.env.DB.prepare(
    "UPDATE letters SET status=?, updated_at=datetime('now','+9 hours') WHERE id=? AND owner_id=?"
  )
...
app.post("/api/admin/signups", requireAuth, async (c) => {
  ...
  await c.env.DB.prepare(
    'INSERT INTO settings(key,value) VALUES("signups_enabled",?) ON CONFLICT(key) DO UPDATE SET value=excluded.value'
  )
